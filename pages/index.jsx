...
const fetchBinaryPrecipData = async (periodParam, timeParam, subsample = 1) => {
  const response = await fetch(
    `${API_BASE_URL}/api/precipitation/binary?period=${periodParam}&time=${timeParam}&subsample=${subsample}`
  );
...
const fetchAggregatedPrecipData = async (periodParam, startTime, endTime, subsample = 1) => {
  const response = await fetch(
    `${API_BASE_URL}/api/precipitation/aggregated/binary?period=${periodParam}&start_time=${startTime}&end_time=${endTime}&subsample=${subsample}`
  );
  const buffer = await response.arrayBuffer();
  return parseBinaryPrecipData(buffer);
};
...
export default function Home() {
  ...
  const startPrefetch = (targetPeriod) => {
    ...
    fetch(`${API_BASE_URL}/api/prefetch?period=${targetPeriod}&subsample=1`, {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/api/periods`)
      .then(res => res.json())
      .then(data => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/times?period=${period}`)
      .then(res => res.json())
      .then(data => {
